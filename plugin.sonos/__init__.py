	...
	def resolve_cmd(self, item, attr):
		if '<sonos_uid>' in item.conf[attr]:
			...
			if uid:
				...
				logger.warning("sonos: could not resolve sonos_uid for {0} from parent item {1}".format(item, parent_item))
...
class SonosCommand():
	...
	def get_uid_from_response(self, dom):
		try:
			return dom.attributes["uid"].value
		except:
			return None

	def get_bool_result(self, dom, result_string):
		try:
